...
def test_rules_audit_detects_sensitive_address_typo_name() -> None:
    result = rules_audit("SELECT adress_ad FROM public.clients LIMIT 10")
...
def test_rules_audit_detects_postgres_invalid_dml_limit() -> None:
    result = rules_audit(
        "UPDATE public.users SET role = 'admin' WHERE id = 1 LIMIT 1"
    )

    assert any(
        f.vulnerability_class == VulnerabilityClass.UPDATE_DELETE_WITHOUT_WHERE
        and "LIMIT в UPDATE/DELETE" in f.explanation
        for f in result.findings
    )
